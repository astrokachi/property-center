...
import { ServiceProfile, AccommodationProfile } from "../types/profile";

export const ListingContainer = () => {
  const [services, setServices] = useState<ServiceProfile[]>([]);
  const [accommodations, setAccommodations] = useState<AccommodationProfile[]>(
    []
  );
  ...
  useEffect(() => {
    if (store.auth.user?.ServiceDetails) {
      setServices(store.auth.user.ServiceDetails);
    }
    if (store.auth.user?.AccommodationDetails) {
